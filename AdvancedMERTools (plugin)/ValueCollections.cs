...
namespace AdvancedMERTools
{
    ...
    public class String : Value
    {
        ...
        public override object GetValue(FunctionArgument args)
        {
                args.PrintDebug($"string start: {Value}");
            ...
            var orig = (string)Value.Clone();
            if (orig != str)
                args.PrintDebug($"string: {orig} -> {str}");
            return str;
        }
    }
    ...
    public class Compare : Value
    {
        ...
        public override object GetValue(FunctionArgument args)
        {
            args.PrintDebug($"CMP start {Operator}", +1);
            object v1 = Value1.GetValue(args);
            object v2 = Value2.GetValue(args);
            if (v1 == null || v2 == null) {
                args.PrintDebug($"CMP: {(v1 == null ? "v2" : "v2")} is null");
                return false;
            }
            if (Operator == CompareType.TypeEqual) {
                args.PrintDebug($"CMP TypeEqual: {v1.GetType()} {v2.GetType()}");
            ...
            if ((v1 is int || v1 is float) && (v2 is int || v2 is float))
            {
                ...
                args.PrintDebug($"CMP num: {V1} {Operator} {V2}");
                switch (Operator)
                {
            ...
            if (v1 is bool && Operator == CompareType.Not) {
                args.PrintDebug($"CMP not: {v1} => {!Convert.ToBoolean(v1)}");
            ...
            if (v1 is bool && v2 is bool)
            {
                ...
                args.PrintDebug($"CMP bool: {V1} {Operator} {V2}");
                switch (Operator)
                {
                    case CompareType.Equal:
                    case CompareType.BigOrEqual:
            ...
            {
                args.PrintDebug($"CMP equal: {v1}[{v1.GetType().Name}] {Operator} {v2}[{v2.GetType().Name}]");
            ...
            args.PrintDebug($"CMP skip FALSE");
            return false;
        }
    }
    ...
    public class Variable : Value
    {
        ...
        public override object GetValue(FunctionArgument args)
        {
            args.PrintDebug($"GET_VAR start", +1);
            ...
            {
                case 0:
                    val = args.Function.FunctionVariables.TryGetValue(str, out object value) ? value : null;
                    scope = "func";
                    break;
                case 1:
                    val = args.Function.ScriptVariables.TryGetValue(str, out value) ? value : null;
                    scope = "script";
                    break;
                case 2:
                    val = AdvancedMERTools.Singleton.SchematicVariables[args.schematic].TryGetValue(str, out value) ? value : null;
                    scope = "schem";
                    break;
                case 3:
                    val = AdvancedMERTools.Singleton.RoundVariable.TryGetValue(str, out value) ? value : null;
                    scope = "game";
                    break;
            }
            args.DebugIndent -= 1;
            if (val == null)
                args.PrintDebug($"GET_VAR[{scope}] {str}=null", -1);
            else
                args.PrintDebug($"GET_VAR[{scope}] {str}={val}[{val.GetType().Name}]", -1);
            return val;
        }
    }
    ...
    public class NumUnaryOp : Value
    {
        ...
        public override object GetValue(FunctionArgument args)
        {
            args.PrintDebug($"NUM UNARY: start {Operator}", +1);
            ...
            if (v is int || v is float)
            {
                ...
                args.DebugIndent -= 1;
                args.PrintDebug($"NUM UNARY: {V} {Operator}");
                switch (Operator)
                {
    ...
    public class NumBinomialOp : Value
    {
        ...
        public override object GetValue(FunctionArgument args)
        {
            args.PrintDebug($"NUM BIN: start {Operator}", +1);
            object v1 = Value1.GetValue(args);
            object v2 = Value2.GetValue(args);
            if (v1 == null || v2 == null)
                return 0;
            if ((v1 is int || v1 is float) && (v2 is int || v2 is float))
            {
                ...
                args.DebugIndent -= 1;
                args.PrintDebug($"NUM BIN: {i} {Operator} {j}");
                switch (Operator)
                {
    ...
    public class ArrUnaryOp : Value
    {
        ...
        public override object GetValue(FunctionArgument args)
        {
            args.PrintDebug($"ARRAY UNARY: start {Operator}", +1);
            object v = Array.GetValue(args);
            args.PrintDebug($"ARRAY UNARY: array is {(v is null ? "null" : v.GetType().Name)}");
            if (v != null && v is object[])
            {
                object[] val = v as object[];
                args.DebugIndent -= 1;
                args.PrintDebug($" values: {string.Join(", ", val.Select(i => $"{i}[{i.GetType().Name}]"))}");
                switch (Operator)
                {
        ...
        public override object GetValue(FunctionArgument args)
        {
            ...
            if (v != null && v is object[])
            {
                ...
                if (Prevaluate.Contains(Operator))
                {
                    ...
                    if (s == null)
                        return a;
                    args.PrintDebug($"ARRAY BINARY: {Operator} ({s}[{s.GetType().Name}])");
                    args.PrintDebug($" values: {string.Join(", ", a.Select(i => $"{i}[{i.GetType().Name}]"))}");
                    switch (Operator)
                    {
                ...
                {
                    args.PrintDebug($"ARRAY BINARY: {Operator}");
                    args.PrintDebug($" values: {string.Join(", ", a.Select(i => $"{i}[{i.GetType().Name}]"))}");
                    ...
                    args.PrintDebug($" evalueted: {string.Join(", ", evaluated.Select(i => $"{i}[{i.GetType().Name}]"))}");
                    switch (Operator)
                    {
