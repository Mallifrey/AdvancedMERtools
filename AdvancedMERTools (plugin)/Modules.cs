...
namespace AdvancedMERTools
{
    ...
    public class FExplodeModule : FRandomExecutionModule
    {
        ...
        public override void Execute(FunctionArgument args)
        {
            ...
            args.PrintDebug($"FExplodeModule:");
            args.PrintDebug($" EffectOnly: {EffectOnly.GetValue(args, true)}");
            args.PrintDebug($" LocalPosition: {LocalPosition.GetValue(args, Vector3.zero)}");
            args.PrintDebug($" FFOn: {FFon.GetValue(args, false)}");
            MEC.Timing.CallDelayed(ActionDelay.GetValue(args, 0f), () =>
            {
    ...
    public class FCFEModule : FRandomExecutionModule
    {
        ...
        public override void Execute(FunctionArgument args) {
            var schemObject = args.schematic;
            if (schemObject == null) Log.Error("FCFEModule: SchematicObject is null");
            MEC.Timing.CallDelayed(ActionDelay.GetValue(args, 0f), () => {
                if (AdvancedMERTools.Singleton.FunctionExecutors[schemObject].TryGetValue(FunctionName.GetValue(args, ""), out FunctionExecutor function))
                ...
                else args.PrintDebug($"function not found: {FunctionName.GetValue(args, "")}");
            });
        }
    }
